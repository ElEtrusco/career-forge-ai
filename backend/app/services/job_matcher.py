...
class JobMatcher:
    ...
    def analyze_job_profile(
        ...
        """
        Analyze a job profile once so it can be reused across
        multiple CV comparisons.

        This is especially useful when comparing the original CV
        and an AI-improved CV against the same job offer.
        """
        ...
        return self.profile_analyzer.analyze_sync(
            job_text
        )
    ...
    def match(
        ...
        job_profile:
            Optional previously analyzed job profile.

            If supplied, the matcher reuses it instead of analyzing
            the same job description again.
        ...
        job_skills = self.extract_skills_from_text(
            job_text
        )
    ...
    def match_cv_text(
        ...
        If job_profile is supplied, it is reused instead of analyzing
